#!/usr/bin/env python3
# Client ID: 714604065199226911
# Bot Token: NzE0NjA0MDY1MTk5MjI2OTEx.XsxFDQ.YuNMxPWGNxu8j0RXqFVpS_FBGuw
# Permissions: 469773362
...
async def on_message(message):
    ...
    if "!hostgo" in message.content.lower():
        channel = client.get_channel(715776396751929446)
    ...
    if "!start" in message.content.lower() and str(message.channel) in permitted:
        ...
        for i in range(3):
            #mafia
            channel = client.get_channel(714890540315050075)
        ...
        for i in range(1):
            #parity
            i = i + 3
            channel = client.get_channel(714890698452631614)
            await channel.set_permissions(users_[i], read_messages=True, send_messages=True)
            paritymemList.append(nicks_[i])
        for i in range(1):
            #medic
            i = i + 4
            channel = client.get_channel(714890970897973258)
            await channel.set_permissions(users_[i], read_messages=True, send_messages=True)
            medicmemList.append(nicks_[i])
        for i in range(1):
            #vig
            i = i + 5
            channel = client.get_channel(714890750701076580)
            await channel.set_permissions(users_[i], read_messages=True, send_messages=True)
    ...
    if "!lynch" in message.content.lower() and permitted in str(message.channel):
        ...
        if nameString.lower() in guildString.lower():
            ...
            await channel.set_permissions(user, read_messages=False, send_messages=False)
            #deadchat channel
            channel = client.get_channel(715741817584353290)
    ...
    if gameEnd is 1:
        ...
        guild = get(client.guilds, id=700087170962292767)
...
client.run("NzE0NjA0MDY1MTk5MjI2OTEx.XsxFDQ.YuNMxPWGNxu8j0RXqFVpS_FBGuw")